...
class GooseController {
    ...
    loadMod(mod) {
        ...
        if (mod1.getShouldInject()) {
            const originalMethod = Goose.prototype[mod1.method];
            Goose.prototype[mod1.method] = function () {
                originalMethod.call(this);
                eval(mod1.code);
            }
        }
        if (mod1.getShouldOverride()) {
            Goose.prototype[mod1.method2] = function () {
                eval(mod1.code2);
            }
        }
        this.goose.main = function () { mod1.onTick() }
    }
}
...
class TestMod extends GooseMod {
    
    onEnable() {
        super.inject("hideGoose", "alert('inject')");
        super.override("showGoose", "alert('override')")
    }
...
class ModTest extends GooseMod {
    
    onEnable() {
        super.override("hideGoose", "alert('test1')");
    }
